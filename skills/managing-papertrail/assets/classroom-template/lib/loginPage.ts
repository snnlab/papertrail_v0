...
// (see middleware.ts's top-of-file comment). Only the instructor ever sees
// this page — students authenticate to this template with a bearer token
// used exclusively by submit.py's HTTP client, never a browser.
export function loginPageHtml(showInvalidPassword = false): string {
  return `<!doctype html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="robots" content="noindex">
<title>Roster — instructor sign-in</title>
<style>
...
<form method="POST" action="/api/login">
  ...
  <button type="submit">Open roster</button>
</form></body></html>`;
}
