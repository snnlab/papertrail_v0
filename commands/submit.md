...
5. **Submit.** On the first submission, run `python3 <script> --url <url> --token <token> [--course <courseId>]` — this saves the classroom config locally and submits in the same call, immediately after the student's go-ahead from step 4. On every later submission, `--url`/`--token` are no longer needed — the saved config is reused automatically; run `python3 <script>` bare. Handle the result:
   ...
   **Instructor feedback, surfaced automatically.** On a created or replayed submission, the script also checks the student's own submitted work for instructor comments (`GET /api/comments`, authenticated with the student's own token — never anyone else's) and prints any not already shown on a prior run under an "Instructor feedback (N new)" heading, each with the commenter, the quoted passage if any, and the comment text. There is no push notification anywhere in this tool — no email, no webhook — so this check-on-submit is the only way a student learns about a comment, matching how `/papertrail:board`'s hosted-collaborator flow already only surfaces comments when the student's own session pulls them. If this section appears, read it out to the student plainly and point them at `/papertrail:board` for full context or to draft a reply; do not treat it as part of the submission's own result (a comment section with no new comments is normal and silent).
